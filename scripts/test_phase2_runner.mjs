...
async function runPhase2Tests() {
  ...
  // ----------------------------------------------------
  console.log('\n▶ [TEST 7] Testing Auth Edge Cases (Invalid Password & Session Expiry)...');
  try {
    // Wrong password
    ...
    // Sign out
    const { error: outErr } = await clientA.auth.signOut();
    assert(!outErr, 'User A successfully signed out and invalidated local session');
  } catch (e) {
